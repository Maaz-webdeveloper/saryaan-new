...
const segColors = ["#20D5E1", "#20D5E1", "#20D5E1"];

// Off colors (Muted colors for background segments)
const segOff = ["#8CE1E9", "#8CE1E9", "#8CE1E9"];
  ...
  form.addEventListener("submit", function (event) {
    ...
    try {
      const existing = JSON.parse(
        localStorage.getItem(waitlistStorageKey) || "[]",
      );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  contactForm.addEventListener("submit", function (event) {
    ...
    const region = (
      document.getElementById("contact-region")?.value || ""
    ).trim();
    const projectStage = (
      document.getElementById("contact-project")?.value || ""
    ).trim();
    const source = (
      document.getElementById("contact-source")?.value || ""
    ).trim();
    const selectedSolutions = Array.from(
      document.querySelectorAll(".contact-solution:checked"),
    ).map((el) => el.value);
    ...
    if (
      ...
      if (contactError) {
        contactError.textContent =
          "Please fill all required contact fields marked with *.";
    ...
    try {
      const existing = JSON.parse(
        localStorage.getItem(contactStorageKey) || "[]",
      );
      const allSubmissions = Array.isArray(existing) ? existing : [];
      allSubmissions.push(payload);
    ...
    entries.forEach((entry, index) => {
      if (entry.isIntersecting) {
        ...
        setTimeout(
          ...
          (index % 3) * 150,
        ); // Mobile par sequence maintain karne ke liye %3
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .querySelectorAll(".reveal-left-careful, .reveal-form-careful")
    .forEach((el) => {
      observer.observe(el);
    });
});
document.addEventListener("DOMContentLoaded", () => {
  ...
  const elementsToAnimate = document.querySelectorAll(
    ".reveal-left-careful, .reveal-form-careful, .reveal-cta",
  );
...
document.addEventListener("DOMContentLoaded", () => {
  const footerObserver = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          const footerItems = entry.target.querySelectorAll(
            ".reveal-footer-item",
          );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Observe elements
  document
    .querySelectorAll(".reveal-item, .reveal-card, #svg-container")
    .forEach((el) => {
      observer.observe(el);
    });
});
